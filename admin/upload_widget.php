...
function render_image_upload_widget($config = []) {
    ...
    $grid_id     = 'grid_' . $widget_id;
    $queue_id    = 'queue_' . $widget_id;
    ...
    <div class="image-upload-widget">
        <label class="form-label">
            ...
            <?php endif; ?>
            <?php if ($multiple): ?>
                <span style="font-weight:400; text-transform:none; color:#94a3b8; letter-spacing:0;">(multiple)</span>
            <?php endif; ?>
        </label>
        ...
        <div class="upload-drop-zone" id="<?php echo $drop_id; ?>">
            <input type="file" id="<?php echo $file_id; ?>" accept="image/*" <?php echo $multiple_attr; ?>>
            <span class="upload-icon">📸</span>
            <div class="upload-text"><?php echo $multiple ? 'Click to browse or drag & drop multiple images' : 'Click to browse or drag & drop an image'; ?></div>
            <div class="upload-subtext">JPG, PNG, WebP, GIF — max 10MB each</div>
        </div>
        ...
        <div class="upload-queue" id="<?php echo $queue_id; ?>"></div>
        <div class="upload-error" id="<?php echo $error_id; ?>"></div>

        <?php if ($multiple): ?>
            <div class="upload-preview-grid" id="<?php echo $grid_id; ?>"></div>
        <?php endif; ?>
    ...
    (function() {
        ...
        function init() {
            initImageUploadWidget({
                ...
                previewGridId: '<?php echo $grid_id; ?>',
                queueId:       '<?php echo $queue_id; ?>',
